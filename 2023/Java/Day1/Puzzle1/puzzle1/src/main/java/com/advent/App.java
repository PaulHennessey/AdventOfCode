...
public final class App {
    ...
    private static List<String> numericDigits = List.of("1", "2", "3", "4", "5", "6", "7", "8", "9");
    private static List<String> alphaDigits = List.of("one", "two", "three", "four", "five", "six", "seven", "eight", "nine");
    private static Map<String, Character> dictDigits = Map.of("one", '1', "two", '2', "three", '3', "four", '4', "five", '5', "six", '6', "seven", '7', "eight", '8', "nine", '9');

    public static void main(String[] args) 
    {
        System.out.println(getSum(fullList));
    }

    public static int getSum(List<String> items)
    {
        Integer sum = 0;
    ...
    {
        return Integer.parseInt("" + getFirstDigit(item) + getLastDigit(item));
    }

    private static char getFirstDigit(String item)
    {   
        int firstIndex = 999;
        char firstDigit = '9';
        
        for(String s : numericDigits)
        {
            if(item.indexOf(s) != -1 && item.indexOf(s) < firstIndex)
            {
                firstIndex = item.indexOf(s);
                firstDigit = item.charAt(firstIndex);  
            }
        }

        for(String s : alphaDigits)
        {
            if(item.indexOf(s) != -1 && item.indexOf(s) < firstIndex)
            {
                firstIndex = item.indexOf(s);
                firstDigit = dictDigits.get(s);
            }
        }
    ...
    private static Character getLastDigit(String item)
    {   
        int lastIndex = -1;
        char lastDigit = '9';
        ...
        for(String s : numericDigits)
        {
            if(item.lastIndexOf(s) != -1 && item.lastIndexOf(s) > lastIndex)
            {
                lastIndex = item.lastIndexOf(s);
                lastDigit = item.charAt(lastIndex);  
            }
        }
    ...
    private static List<String> fullList = List.of(
    "threehqv2",
