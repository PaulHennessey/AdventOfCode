...
public class App {
    ...
    {
        System.out.println(ProcessGamesPower(fullList));
    }

    public int ProcessGamesInheritance(List<Game> games)
    {
        int total = 0;
    ...
    public int ProcessGames(List<String> games)
    {
        int total = 0;
        String gameBody;
        int gameId;
        ...
        {
            String[] gameHeaderBody = game.trim().split(":");
            gameBody = gameHeaderBody[1];

            String[] gameName = gameHeaderBody[0].split(" ");
            gameId = Integer.parseInt(gameName[1]);

            if(ProcessGame(gameBody))
                total += gameId;
        }
    ...
    public int ProcessGamesPower(List<String> games)
    {
        int total = 0;
        String gameBody;
        ...
        for(String game: games)
        {
            ...
            total += ProcessGamePower(gameBody);
        }
        
        return total;
    }

    public boolean ProcessGame(String game)
    {
        String[] sets = game.trim().split(";");
        
        for(String set: sets)
        {
            if(ProcessSet(set) == false)
                return false;
        }
    ...
    public int ProcessGamePower(String game)
    {
        int red, green, blue;
        red = green = blue = 0;
        ...
        for(String set: sets)
        {
            String[] colours = set.trim().split(", ");

            for(String colour: colours)
            {
                String[] colourValue = colour.split(" ");

                switch (colourValue[1]) {
                    case "red":
                        int r = Integer.parseInt(colourValue[0]);
                        if(r > red)
                            red = r;
                        break;
                    case "green":
                        int g = Integer.parseInt(colourValue[0]);
                        if(g > green)
                            green = g;
                        break;
                    case "blue":
                        int b = Integer.parseInt(colourValue[0]);
                        if(b > blue)
                            blue = b;
                        break;
                }
    ...
    public boolean ProcessSet(String set)
    {
        int red, green, blue;
        red = green = blue = 0;

        String[] colours = set.trim().split("\\s*, ");
        ...
        for(String colour: colours)
        {
            ...
            switch (colourValue[1]) {
                case "red":
                    red = Integer.parseInt(colourValue[0]);
                    break;
                case "green":
                    green = Integer.parseInt(colourValue[0]);
                    break;
                case "blue":
                    blue = Integer.parseInt(colourValue[0]);
                    break;
            }
        }

        return Possible(red, green, blue);
    }

    public boolean Possible(int red, int green, int blue)
    {
        return red <= RED && green <= GREEN && blue <= BLUE;
    }

    private List<String> fullList = List.of(
